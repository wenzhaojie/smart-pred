...
class Fusion_segment_bursty_model(Basic_model):
    def __init__(self,  fusion_model_params_dict=None, name="Fusion_model"):
        super(Fusion_segment_bursty_model, self).__init__(name=name)
        self.default_extra_parameters = {
            ...
            "delta_t": 240,
            "epsilon": 1.2,
            "bursty_quantile": 0.95,
        }
        self.fusion_model_params_dict = fusion_model_params_dict


    def init_fusion_model_params_dict(self, fusion_model_params_dict):
        self.fusion_model_params_dict = fusion_model_params_dict
    ...
    def use_future_rolling_evaluation(self, train, test, extra_parameters=None):
        ...
        # 对bursty的部分进行处理，用maxvalue_model进行处理
        bursty_quantile = extra_parameters["bursty_quantile"]
        # bursty的衡量是根据train最后一个周期，然后进行差分进行的
        to_diff_observation_trace = train[-extra_parameters["period_length"]:]
        # 计算diff_observation_trace
        diff_observation_trace = np.diff(to_diff_observation_trace)
        # 前面补充一个0
        diff_observation_trace = np.insert(diff_observation_trace, 0, 0)
        # 计算quantile
        quantile = np.quantile(diff_observation_trace, bursty_quantile)
        # 找到bursty的时间点
        bursty_time_point = []
        for i in range(len(diff_observation_trace)):
            if diff_observation_trace[i] > quantile:
                bursty_time_point.append(i)
        # 对bursty的时间点进行处理，使用maxvalue_model进行处理
        maxvalue_model = fusion_model_dict["Maxvalue_model"]
        maxvalue_model_extra_parameters = self.fusion_model_params_dict["Maxvalue_model"]
        _, maxvalue_model_predict = maxvalue_model.use_future_rolling_evaluation(
            train=train,
            test=test,
            extra_parameters=maxvalue_model_extra_parameters
        )
        # 对bursty的时间点进行处理，修正为与之前预测值和maxvalue_model的最大值
        for i in bursty_time_point:
            predict_list[i] = max(predict_list[i], maxvalue_model_predict[i])
