...
class NeuralForecast_model(Basic_model):
    ...
    def train(self, history_df, extra_parameters=None):
        # 确保 unique_id, y, ds 列存在
        assert "unique_id" in history_df.columns
        assert "y" in history_df.columns
        assert "ds" in history_df.columns
        ...
        # input_df = deepcopy history_df
        input_df = deepcopy(history_df)

        # 如果有缩放器且启用了缩放
        if self.scaler and extra_parameters["is_scaler"]:
            input_df['y'] = self.scaler.fit_transform(history_df[['y']])
    ...
    def predict(self, history_df, predict_window, extra_parameters=None):
        ...
        # 预测
        Y_hat_df = self.nf.predict(df=input_df).reset_index()
...
def Test():
    ...
    history_df = pd.DataFrame(
        {
            'y': history,
            'ds': pd.date_range(start='2000-01-01', periods=len(history), freq='H'),
    ...
    for model_name in model_names:
        ...
        model.train(history_df=history_df, extra_parameters=extra_parameters)

        predict = model.predict(history_df=history_df, predict_window=pred_len, extra_parameters=extra_parameters)
