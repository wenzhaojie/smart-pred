...
# 八中不同的bar颜色
my_plotter.color_list = ['#43978F', '#9EC4BE', '#ABD0F1', '#DCE9F4', '#E56F5E', '#F19685', '#F6C957', '#FFB77F', '#FBE8D5']

my_plotter.edge_color_list = ['w', 'w', 'w', 'w', 'w', 'w', 'w', 'w', 'w']
...
def stat_all_data():
    res_dict_list = []
    for method in ["Avgvalue", "Maxvalue", "NBEATS", "Movingavg", "Movingmax", "NHITS", "PatchTST", "TimesNet"]:
        for pattern in ["continuous", "sparse", "period", "bursty"]:
            res_dict = get_pattern_method_avg_mae(pattern, method, "./plot_trace/2024-05-19-20-48-21")
            print(res_dict)
...
def plot_pattern_method_avg_mae():
    ...
    my_plotter.plot_bars(
        ...
        legend_title=None,
        save_root=save_root,
        filename="plot_pattern_method_avg_mae.pdf",
        x_label_rotation=45,
    )
...
def plot_method_pattern_avg_mae():
    ...
    my_plotter.plot_bars(
        ...
        legend_title=None,
        save_root=save_root,
        filename="plot_method_pattern_avg_mae.pdf",
    )
