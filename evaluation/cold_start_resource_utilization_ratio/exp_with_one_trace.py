...
class ExpOneTrace:
    ...
    # 准备好测试一个trace所需的所有东西
    # 包括模型对象，模型超参数配置
    def __init__(self, dataset_name="HuaweiPrivateDataset", function_name="28", start_day=0, end_day=4):
    ...
    def init_baselines(self, baselines_name_list=None, baselines_config_dict=None, ):

        self.baselines_dict = {
            "Keepalive": Movingmax_model(),
            ...
            "Movingmin": Movingmin_model(),
            "Maxvalue": Maxvalue_model(),
            "Minvalue": Minvalue_model(),
            "Avgvalue": Avgvalue_model(),
            "Quantile": Quantile_model(),
        }
        if baselines_name_list is not None:
            # self.baselines_dict 中只保留 baselines_name_list 中的模型
            self.baselines_dict = {
                k: v for k, v in self.baselines_dict.items() if k in baselines_name_list
            }
        ...
        if baselines_config_dict is None:
            ...
            self.baselines_config_dict = baselines_config_dict
    ...
    def evaluation(self):
        ...
        # 保存实验参数字典
        # 为当前实验创建一个文件夹
        save_root = os.path.join("results", f"{self.dataset_name}_{self.function_name}_{self.start_day}_{self.end_day}")
        save_json_path = f"{save_root}/experiment_parameters.json"
        # 创建文件夹
        ...
        # 也保存一个版本csv
        # 字典去掉这 几个key，"dataset_name", "function_name", "start_day", "end_day",
        dict_obj_removed = {k: v for k, v in experiment_parameters_dict.items() if k not in ["dataset_name", "function_name", "start_day", "end_day"]}
        # 第一列 是 dict_obj_removed的所有 key（method），对应的value是一个新的dict，key是metric，value是metric的值
        # 最终我要的csv 第一列名称是method，第二列到最后一列分别对应metric的值

        df = pd.DataFrame.from_dict(dict_obj_removed, orient='index')
        df.reset_index(inplace=True)
        df.rename(columns={'index': 'method'}, inplace=True)

        # 保存到CSV文件
        save_csv_path = f"{save_root}/experiment_parameters.csv"
        ...
        # 同时导出train 真实值 和 test 真实值
        save_npy_path = f"{save_npy_root}/train_true.npy"
        np.save(save_npy_path, self.train_data)
        save_npy_path = f"{save_npy_root}/test_true.npy"
        np.save(save_npy_path, self.test_data)
    ...
    @staticmethod
    def result_analysis(self, experiment_parameters_dict):
        # 分析experiment_parameters_dict
        pass
